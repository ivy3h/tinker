...
if not os.path.exists(log_path):
    ...
    os.system(
        "find /srv/nlprx-lab/share6/jhe478/tinker-cookbook -name 'metrics.jsonl' 2>/dev/null | head -10"
    )
    exit(1)
...
print(
    f"  Improvement:         {(1 - df['train_mean_nll'].iloc[-1] / df['train_mean_nll'].iloc[0]) * 100:.1f}%"
)
...
print("\n⏱️ Training Time:")
print(
    f"  Total training time: {df['time/total'].sum():.2f} seconds ({df['time/total'].sum() / 60:.2f} minutes)"
)
...
with open(report_file, "w") as f:
    ...
    f.write(
        f"  Loss reduction:      {loss_reduction:.4f} ({(1 - df['train_mean_nll'].iloc[-1] / df['train_mean_nll'].iloc[0]) * 100:.1f}%)\n\n"
    )
